...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ orderNumber: string }> }) {
  ...
  const doc = createElement(GstInvoiceDoc, {
    ...
    buyer: {
      name: order.user?.name ?? shippingAddr.fullName ?? shippingAddr.name ?? 'Customer',
      address: [shippingAddr.address1 || shippingAddr.address, shippingAddr.city, shippingAddr.pincode].filter(Boolean).join(', '),
